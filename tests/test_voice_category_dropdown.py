...
修复：``ui.voice_handlers._save_category_choices`` 保证“未分类”始终在保存分类下拉的
...
def test_save_category_choices_empty_keeps_default():
    choices = voice_ui._save_category_choices([])
    assert choices == ["未分类", "— 新建 —"]


def test_save_category_choices_none_keeps_default():
    choices = voice_ui._save_category_choices(None)
    assert choices == ["未分类", "— 新建 —"]
...
def test_refresh_categories_value_is_in_choices(monkeypatch):
    ...
    monkeypatch.setattr(voice_ui.voice_lib, "list_categories", lambda: ["温柔"])
    bind_update, save_update = voice_ui.refresh_categories()
...
def test_refresh_voice_filters_value_is_in_choices(monkeypatch):
    monkeypatch.setattr(voice_ui.voice_lib, "list_categories", lambda: ["低沉", "温柔"])
    bind_update, lib_update, save_update = voice_ui.refresh_voice_filters()
    assert save_update["value"] == "未分类"
    assert "未分类" in save_update["choices"]
...
def test_refresh_voice_filters_empty_library_keeps_uncategorized(monkeypatch):
    monkeypatch.setattr(voice_ui.voice_lib, "list_categories", lambda: [])
    bind_update, lib_update, save_update = voice_ui.refresh_voice_filters()
...
def test_save_to_lib_refresh_keeps_value_in_choices(monkeypatch):
    ...
    monkeypatch.setattr(voice_ui.voice_lib, "list_categories", lambda: ["温柔"])
    monkeypatch.setattr(
        voice_ui.ProjectService,
        "save_to_lib",
        staticmethod(lambda recorded, uploaded, name, category="": "/tmp/lib/温柔_x.wav"),
    )
    msg, _lib_update, _browser_update, save_update = voice_ui.save_to_lib(

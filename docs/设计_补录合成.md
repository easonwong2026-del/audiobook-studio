...
- `tests/test_supplement.py` — 引擎锁、`build_bound_role_choices`、`export_supplement`、SupplementService 全量测试（monkeypatch 哑 wav 桩）。
- `tests/test_supplement_wiring.py` — nav 按钮 / grp 显隐 / 接线存在（AST 风格，对齐 `test_app_wiring.py`）。
- `tests/qa_supplement_boundary.py` — QA 边界验证脚本（非 `test_*` 前缀，不被 pytest 误收集）。
- `tests/qa_verify_script_loader_fix.py` / `tests/test_script_loader_diagnostics.py` — 上一轮「剧本校验失败」诊断修复的回归测试（与本功能同源）。
...
- `lib/project_manager.py` — 新增 `build_bound_role_choices(script, bindings) -> list[tuple]`（仅 `bindings.get(role)` 为真值的角色，标签前缀「【已绑定】」）。
...
- **T1【P0·基础】引擎互斥锁 + 已绑定角色下拉助手**
  - `lib/tts_engine.py`（锁）、`lib/project_manager.py`（`build_bound_role_choices`）+ 测试。无依赖。
- **T2【P0·核心】导出薄封装 + 补录服务**
...
3. **角色下拉刷新约定**：补录角色下拉 `sup_role` 只列「已绑定音色角色」，用 `project_manager.build_bound_role_choices(script, bindings)`；刷新时机 = `nav_supplement.click` 时 `refresh_supplement_roles(ss)` **懒刷新**；**不修改** `open_project_full` 的 `_FULL_OUTPUTS` 22 元组契约（避免牵动全量刷新链路），未开项目时 `sup_role.interactive=False` 并提示「请先打开项目并绑定角色音色」。
